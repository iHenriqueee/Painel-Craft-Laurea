// Laurea boot flags + crash reporting (added for Pages debug)
window.__LAUREA_APP_LOADED = true;
window.addEventListener('error', function(e){
  try {
    var el = document.getElementById('statusBar');
    if(el){
      var msg = (e && (e.message || (e.error && e.error.message))) || 'Erro JS';
...
window.addEventListener('unhandledrejection', function(e){
  try {
    var el = document.getElementById('statusBar');
    if(el){
      var msg = (e && e.reason && (e.reason.message || String(e.reason))) || 'Promise rejeitada';
      el.innerHTML = '<strong>ERRO</strong>: ' + msg;
    }
  } catch(_){}
});
document.addEventListener('DOMContentLoaded', function(){
  try {
    var el = document.getElementById('statusBar');
    if(el && el.textContent && el.textContent.indexOf('JS') !== -1){
...
(function(){
  ...
  function loadState(){
    ...
    try{
      ...
      if (typeof st.desconto !== 'undefined' && (typeof st.descontoExtra === 'undefined')){
        var d = Number(st.desconto) || 0;
        descontoExtra = Math.max(0, Math.min(100, d));
      }
...
// ---------- Modal (admin products) ----------
  ...
  if (resetBtn) resetBtn.addEventListener('click', function(){
    ...
    if (!confirm('Resetar tudo?\nIsso apaga produtos/preços/itens/desconto extra do aparelho.')) return;
    lsRemove(STATE_KEY);
    alert('Reset feito ✅\nA página vai recarregar.');
    location.reload();
  });
