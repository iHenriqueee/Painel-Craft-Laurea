(function(){
  ...
  var baseDescontos = { parceria: 0, alianca: 0 }; // admin
  var descontoExtra = 0; // user
  var saleMode = 'pista';
  var priceTab = 'pista';
  var itensState = null;
  ...
  function loadState(){
    ...
    try{
      ...
      if (st.baseDescontos && typeof st.baseDescontos === 'object') baseDescontos = st.baseDescontos;
  ...
  function getState(){
    return { theme: getCurrentTheme(), receitas: receitas, precos: precos, baseDescontos: baseDescontos, descontoExtra: descontoExtra, saleMode: saleMode, priceTab: priceTab, itens: getItensDaTela() };
  }
  ...
  var baseDescParceria = document.getElementById('baseDescParceria');
  var baseDescAlianca = document.getElementById('baseDescAlianca');
  ...
  function applyPermissions(){
    ...
    if (baseDescParceria) baseDescParceria.disabled = !admin;
    if (baseDescAlianca) baseDescAlianca.disabled = !admin;
  ...
  function calcularFinanceiro(){
    ...
    for (var i=0;i<itens.length;i++){
      ...
      bruto += (Number(obj[saleMode] || 0) || 0) * itens[i].qtd;
    }
    var basePct = 0;
    if (saleMode === 'parceria') basePct = Number(baseDescontos.parceria || 0) || 0;
    if (saleMode === 'alianca') basePct = Number(baseDescontos.alianca || 0) || 0;
    var extraPct = Math.max(0, Math.min(100, Number(descontoExtra || 0) || 0));
    basePct = Math.max(0, Math.min(100, basePct));
    var pct = Math.max(0, Math.min(100, basePct + extraPct));
    ...
    if (itens.length){
      ...
      for (var j=0;j<itens.length;j++){
        ...
        var sub = (Number(pobj[saleMode] || 0) || 0) * it.qtd;
