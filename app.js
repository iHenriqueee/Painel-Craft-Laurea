(function(){
  ...
  // Inputs de preço no modal: permitir "." para milhar e manter cálculo ok.
  var modalPriceFields = [
    precoPistaLimpoProduto, precoPistaSujoProduto,
    precoParceriaLimpoProduto, precoParceriaSujoProduto,
    precoAliancaLimpoProduto, precoAliancaSujoProduto
  ];
  for (var mp=0; mp<modalPriceFields.length; mp++){
    (function(el){
      if (!el) return;
      el.addEventListener('input', function(){ applyLiveMask(el); });
      el.addEventListener('blur', function(){ el.value = formatBRInputLive(el.value); });
    })(modalPriceFields[mp]);
  }
  ...
  function renderPrecos(){
    ...
    function td(n, kind, val){
      return '<div class="col-preco"><input class="precoInp" data-prod="'+ escapeHtml(n) +'" data-mode="'+ mode +'" data-kind="'+ kind +'" type="text" inputmode="decimal" pattern="[0-9.,]*" placeholder="0" value="'+ fmtBRNumber(val||0) +'"></div>';
    }
    ...
    for (var j=0;j<inps.length;j++){
      inps[j].addEventListener('input', function(){
        ...
        // Máscara ao vivo (permite digitar com "." para milhar)
        applyLiveMask(this);
        var val = parseBRNumber(this.value);
        var pr2 = ensurePreco(prod);
        if (!pr2[mode]) pr2[mode] = {limpo:0,sujo:0};
        pr2[mode][kind] = val;
        precos[prod] = pr2;
        // Guarda a última versão válida (formatada) para bloquear usuário não-admin
        this.setAttribute('data-prev', formatBRInputLive(this.value));
        saveState();
        calcularFinanceiro();
      });
      inps[j].setAttribute('data-prev', String(inps[j].value||''));
      inps[j].addEventListener('blur', function(){ this.value = formatBRInputLive(this.value); this.setAttribute('data-prev', this.value); });
    }
  }


  // ---------- Number parsing/formatting (pt-BR friendly) ----------
  // Máscara leve: permite digitar com "." (milhar) e "," (decimal) sem quebrar cálculo.
  // Formata enquanto digita para ficar organizado (ex.: 260000 -> 260.000).
  function formatBRInputLive(raw){
    var s = String(raw || '');
    s = s.replace(/[^\d.,]/g, '');
    if (!s) return '';

    // Decide separador decimal: prioriza vírgula
    ...
    if (hasComma){
      var pieces = s.split(',');
      intPart = pieces[0];
      decPart = (pieces[1] || '').replace(/\D/g, '').slice(0, 2);
    } else if (s.indexOf('.') !== -1){
      // Se tiver ponto e o último grupo tiver 1-2 dígitos, tratamos como decimal
      var p2 = s.split('.');
      if (p2.length >= 2){
        var last = p2[p2.length - 1].replace(/\D/g, '');
        var allBefore = p2.slice(0, -1).join('');
    ...
    intPart = String(intPart || '').replace(/\D/g, '');
    if (!intPart) intPart = '0';

    // agrupa em milhares com ponto
    var out = '';
    for (var i = intPart.length - 1, g = 0; i >= 0; i--, g++){
      out = intPart.charAt(i) + out;
      if (g % 3 === 2 && i !== 0) out = '.' + out;
    }
  ...
  function applyLiveMask(el){
    if (!el) return;
    // Mantém o cursor aproximadamente no lugar, baseado na contagem de dígitos à direita
    var raw = el.value;
    var start = (typeof el.selectionStart === 'number') ? el.selectionStart : raw.length;
    var digitsRight = raw.slice(start).replace(/\D/g,'').length;
    var formatted = formatBRInputLive(raw);
    el.value = formatted;
    // reposiciona cursor contando dígitos a partir do fim
    try{
      var pos = formatted.length;
      var need = digitsRight;
      while (pos > 0 && need > 0){
        pos--;
        if (/\d/.test(formatted.charAt(pos))) need--;
      }
  ...
  function parseBRNumber(raw){
    ...
    if (!str) return 0;
    // remove currency and spaces, keep digits, separators and minus
    str = str.replace(/[^\d.,-]/g, '');
    if (!str || str === '-') return 0;

    // If has comma, treat comma as decimal separator and dot as thousand separator
    if (str.indexOf(',') !== -1){
    ...
    // No comma: dot might be decimal OR thousand separator
    var parts = str.split('.');
    if (parts.length > 2){
      str = parts.join('');
    } else if (parts.length === 2){
      var a = parts[0], b = parts[1];
      // if exactly 3 digits after dot, assume thousand separator
...
function moneyBR(v){
  ...
  function calcularFinanceiro(){
    ...
    modoLabel = String(modoLabel||'').toUpperCase();
  ...
  function renderProdutos(){
    ...
    for (var e=0;e<edits.length;e++){
      edits[e].addEventListener('click', function(){
        ...
        // Mostra já formatado (com pontos) para facilitar edição
        if (precoPistaLimpoProduto) precoPistaLimpoProduto.value = formatBRInputLive(fmtBRNumber(Number(prEdit.pista.limpo || 0) || 0));
        if (precoPistaSujoProduto) precoPistaSujoProduto.value = formatBRInputLive(fmtBRNumber(Number(prEdit.pista.sujo || 0) || 0));
        if (precoParceriaLimpoProduto) precoParceriaLimpoProduto.value = formatBRInputLive(fmtBRNumber(Number(prEdit.parceria.limpo || 0) || 0));
        if (precoParceriaSujoProduto) precoParceriaSujoProduto.value = formatBRInputLive(fmtBRNumber(Number(prEdit.parceria.sujo || 0) || 0));
        if (precoAliancaLimpoProduto) precoAliancaLimpoProduto.value = formatBRInputLive(fmtBRNumber(Number(prEdit.alianca.limpo || 0) || 0));
        if (precoAliancaSujoProduto) precoAliancaSujoProduto.value = formatBRInputLive(fmtBRNumber(Number(prEdit.alianca.sujo || 0) || 0));
...
if (resetBtn) resetBtn.addEventListener('click', function(){
  ...
  if (salvarProdutoBtn) salvarProdutoBtn.addEventListener('click', function(){
    ...
    // Permite números com "." e "," (pt-BR) nos campos de preço
    var pricePistaLimpo = parseBRNumber((precoPistaLimpoProduto && precoPistaLimpoProduto.value) || 0);
    var pricePistaSujo = parseBRNumber((precoPistaSujoProduto && precoPistaSujoProduto.value) || 0);
    var priceParceriaLimpo = parseBRNumber((precoParceriaLimpoProduto && precoParceriaLimpoProduto.value) || 0);
    var priceParceriaSujo = parseBRNumber((precoParceriaSujoProduto && precoParceriaSujoProduto.value) || 0);
    var priceAliancaLimpo = parseBRNumber((precoAliancaLimpoProduto && precoAliancaLimpoProduto.value) || 0);
    var priceAliancaSujo = parseBRNumber((precoAliancaSujoProduto && precoAliancaSujoProduto.value) || 0);
  ...
  function init(){
    ...
    tryLoadRemotePublishedIfEmpty(function(){
      ...
      // Se não houver produtos cadastrados, não travamos a aplicação.
      // A pessoa ainda pode importar um JSON ou (admin) criar produtos no modal.
      if (!first){
        if (listaItens) listaItens.innerHTML = '<div class="emptyHint">Nenhum produto cadastrado. Importe um JSON ou crie produtos em <b>Gerenciar produtos</b>.</div>';
      ...
      renderProdutos();
      applyPermissions();
      if (!first) setStatus('ok', '<strong>JS OK</strong> • pronto • sem produtos');
      else setStatus('ok', '<strong>JS OK</strong> • pronto');
    });
    }catch(e){
