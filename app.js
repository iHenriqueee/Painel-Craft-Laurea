(() => {
  ...
  function getState(){
    return { receitas, precos, desconto, itens: getItensDaTela() };
  }
  ...
  if (themeToggleBtn){
    themeToggleBtn.addEventListener('click', () => {
      ...
      applyTheme(next);
      // atualizar texto/icone
      themeToggleBtn.textContent = next === 'dark' ? '☀️ Tema' : '🌙 Tema';
    });
    // set initial label
    const current = document.documentElement.getAttribute('data-theme') || 'light';
    themeToggleBtn.textContent = current === 'dark' ? '☀️ Tema' : '🌙 Tema';
  }
